...
import com.ari.prasetiyo.dao.daoPresensiAbsensi;
...
public class presensiAbsensi extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse res){
        ...
        if (id == null  ){
            try {
                ...
                */
                ...
                req.getRequestDispatcher("pro/presensi_absensi.jsp").forward(req, res);
            } catch (ServletException ex) {
                Logger.getLogger(presensiAbsensi.class.getName()).log(Level.SEVERE, null, ex);
            } catch (IOException ex) {
                Logger.getLogger(presensiAbsensi.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        //ambil data karyawan untuk absensi
     ...
     public void doPost(HttpServletRequest req, HttpServletResponse res){
       ...
       // }
            ...
            new com.ari.prasetiyo.sistem.loggerError(presensiAbsensi.class.getName(), "bukan save atau edit, pelaku"  + uSer);
        } 
    }
    ...
    private void saveData(HttpServletRequest req, HttpServletResponse res, int  SorE){
        Gson gson = new Gson();
        domainPresensiAbsensi dPA = new domainPresensiAbsensi();
       //fomat json to array
        String sendid_kar = req.getParameter("sendid_kar");
        String sendMasuk = req.getParameter("sendMasuk");
        String sendAbsen = req.getParameter("sendAbsen");
        String sendMT = req.getParameter("sendMT");
        String sendPLA = req.getParameter("sendPLA");
        
        Type type = new TypeToken<String[]>() {}.getType();
        String[] id_kar = gson.fromJson(sendid_kar, type);
        String[] masuk = gson.fromJson(sendMasuk, type);
        String[] absen = gson.fromJson(sendAbsen, type);
        String[] MT = gson.fromJson(sendMT, type);
        String[] PLA = gson.fromJson(sendPLA, type);
        ...
        com.ari.prasetiyo.dao.daoPresensiAbsensi Save = new com.ari.prasetiyo.dao.daoPresensiAbsensi();
     ...
     */
     private void kirimDataAjax (HttpServletResponse response, int LimitBawah, int LimitAtas,  com.ari.prasetiyo.domain.domainMasterPelamarAll dMA ){
        try {
            ...
            /* pagging end */
            /* Data table */
            daoPresensiAbsensi dao = new daoPresensiAbsensi();
            ...
            if (dMA.isLanjut() && dMA.getfPeriodeCreate1() != null  ){
                sql = "select count(id_karyawan) as id "
                        + "from presensi_absensi "
                        + "where id_karyawan like ? and status_karyawan like ? and nama like ? and jabatan like ? "
                        + "and area like ? and status_pegawai like ?    " 
            ...
            //data new absensi
                ...
                sql = "select count(id_karyawan) as id from presensi_absensi where date_format(tanggal_dibuat, '%Y-%m') = date_format(now(), '%Y-%m')";
                jumlahRow = daoQueryAll.jumlahRow(sql, dMA,"filterPresensiAbsensi");
