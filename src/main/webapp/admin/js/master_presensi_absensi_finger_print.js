...
$(document).ready(function() {
   ...
   */
    ...
    $(".b_save").on("click", function(e){
        ...
        for ( var i = 0; i < lastRow; i++) {
            masuk.push($("#masuk"+i).val());  //pushing all the names listed in the table
            absen.push($("#absen"+i).val());  
            MT.push($("#PLA"+i).val()); 
            PLA.push($("#PLA"+i).val()); 
            id_kar.push(document.getElementById("id_kar"+i).getAttribute("itemid"));
        }
        var sendMasuk = JSON.stringify(masuk);  
        var sendAbsen = JSON.stringify(absen);  
        var sendMT = JSON.stringify(MT);  
        var sendPLA = JSON.stringify(PLA);  
        var sendid_kar = JSON.stringify(id_kar);
        var user  = $("b#usernya").text();
        var dataString = "&usernya="+user;
         $.ajax({
            type: "POST",
            url: "presensi_absensi.ari?xcv="+1+"&sta=0"+dataString,
            data: {sendid_kar: sendid_kar, sendMasuk : sendMasuk , sendAbsen : sendAbsen, sendMT: sendMT, sendPLA:sendPLA},
            dataType: "json",
    ...
    function ambilDataListPelamar( limitBawah,  limitAtas){
        ...
        $.ajax({
            type: "GET",
            url: "presensi_absensi.ari?xcv="+pintu+"",
            data:dataString,
            dataType: "json",
            //if received a response from the server
            beforeSend: function(){
            ...
            success: function( data, textStatus, jqXHR) {
                ...
                if ( dataTableLengt > 0){
                    ...
                    if(b_filter_stat === "new"){
                        ...
                        var data_table ="<tr id='id_kar"+i+"' itemid='"+data.dataTable[i].id +"'> <td class='text-center'><a href='#' class='info tbl-edit' id='"+data.dataTable[i].id +"'> "+data.dataTable[i].id+"</a></td>"
                                        ...
                                        +"<td class='text-center' nowrap><input name='nama_ari' class='angka' id='masuk"+i+"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input name='nama_ari' class='angka' id='absen"+i+"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input name='nama_ari' class='angka' id='MT"+i+"' value='0' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input name='nama_ari' class='angka' id='PLA"+i+"' value='0' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input  id='cuti"+i+"' type='text' style='width: 50px' disabled></input></td>"
                                        +"<td class='text-center' nowrap><input  id='sakit"+i+"' type='text' style='width: 50px' disabled></input></td>"
                                        +"<td class='text-center' nowrap><input  id='lembur"+i+"' type='text' style='width: 50px' disabled></input></td>"
                        ...
                        */
                        ...
                        var data_table ="<tr id='id_kar"+i+"' itemid='"+data.dataTable[i].id +"'> <td class='text-center'><a href='#' class='info tbl-edit' id='"+data.dataTable[i].id +"'> "+data.dataTable[i].id+"</a></td>"
                                        ...
                                        +"<td class='text-center' nowrap>"+data.dataTable[i].sts_pegawai+"</td>"
                                        +"<td class='text-center' nowrap>"+data.dataTable[i].statusPelamar+"</td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].masukInt +"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].absenInt +"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].masukTerlambat +"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].pulangLebihAwal +"' type='text' style='width: 50px'></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].cuti +"' type='text' style='width: 50px' disabled></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].sakit +"' type='text' style='width: 50px' disabled></input></td>"
                                        +"<td class='text-center' nowrap><input disabled value='"+data.dataTable[i].lembur +"' type='text' style='width: 50px' disabled></input></td>"
                                        +"<td class='text-center' nowrap><a id='"+data.dataTable[i].id +"' data-toggle='modal' data-target='#v_edit' class='btn btn-primary kryw_edit'><div class='glyphicon glyphicon-edit'></div></a> <a id='kryw_del' class='btn btn-primary'><div class='glyphicon glyphicon-remove'></div></a></td>"
                                        +"</tr>";
                        //$(msg_data).appendTo("#content");
                        $('#ariDataTablePelamar').append(data_table);
   ...
   */
    //tanggal filter hanya periode saja
